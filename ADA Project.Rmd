...
```{r}
model <- glm(obe_dichot ~ shp, data=Capstone_analysis, family=binomial)

#define new data frame that contains predictor variable
newdata <- data.frame(shp=seq(min(Capstone_analysis$shp), max(Capstone_analysis$shp),len=500))
...
lines(obe_dichot ~ shp, newdata, lwd=2)
```
